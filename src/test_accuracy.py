...
def plot_device_accuracy(y, X, agg, metrics_df, device_id, horizon=60, history_len=120):
    # Load model
    ...
    # Metrics
    dev_metrics = metrics_df[metrics_df["DeviceId"] == device_id].iloc[0]
    ...
    print("="*60)
    print(f"- MAPE (Sai so du bao): {mape:.2f}% ({'Tot' if mape < 20 else 'Trung binh/Kem'})")
    print(f"- R2 (Do tuong quan xu huong): {r2:.3f} ({'Tot' if r2 > 0.4 else 'Kem'})")
    print(f"- MAE (Lech so xe trung binh): +/-{mae:.2f} xe/phut")
    ...
    # Forecast
    horizon_steps = min(horizon, test_len)
    ...
    ax1.set_title(f"DOI CHAT TRUC TIEP: Thuc te vs Du bao trong {horizon_steps} phut (MAE: {mae_horizon:.2f} xe/phut)", fontsize=11, fontweight="bold", color="#1e88e5")
    ax1.set_xlabel("Thoi gian", fontsize=9)
    ax1.set_ylabel("Luu luong (xe / phut)", fontsize=9)
...
def main():
    ...
    device_ids = sorted(metrics["DeviceId"].tolist())

    while True:
        print("\nDanh sach cac tram VDS:")
