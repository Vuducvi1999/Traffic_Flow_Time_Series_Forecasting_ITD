...
def is_met(mae, rmse, mape, r2):
    return (
        TARGET_MAE_MIN <= mae <= TARGET_MAE_MAX and
        TARGET_RMSE_MIN <= rmse <= TARGET_RMSE_MAX and
        TARGET_MAPE_MIN <= mape <= TARGET_MAPE_MAX and
        r2 >= TARGET_R2_MIN
    )
...
def try_config(device_id: str, df_device: pd.DataFrame, tier: dict) -> dict:
    df = df_device.set_index("BucketTime").asfreq("1min")
    ...
    y = df["NumVehicles"].values.astype(float)
    ...
    n = len(y)
    test_len = 60
    val_len = tier.get("val_length", 60)
    ...
    if tier.get("use_auto_arima"):
        try:
            s = 60 if tier.get("seasonal") else 1
    ...
    try:
        ...
        return {
            ...
            "success": is_met(mae, rmse, mape, r2),
        }
    except Exception as e:
...
def save_device_config(device_id, config, result):
    if result["success"]:
        ...
        logger.info(f"Device {device_id[:8]}: ✓ with {tier_name} "
        ...
        config["devices"][device_id] = {
            "order": result["order"],
            "seasonal_order": result["seasonal_order"],
            "_source": f"best_of_{result['config_name']}",
            "notes": f"Best effort - MAE={result['mae']:.2f}, R2={result['r2']:.4f}"
        }
        logger.info(f"Device {device_id[:8]}: ✗ best={result['config_name']} "
                    f"(MAE={result['mae']:.2f}, RMSE={result['rmse']:.2f}, "
                    f"MAPE={result['mape']:.2f}%, R2={result['r2']:.4f})")
    save_config(config)

def optimize_one_device(args):
    device_id, df_data, config = args
    df_device = df_data[df_data["DeviceId"] == device_id].copy()

    cur_cfg = config.get("devices", {}).get(device_id, {})
    order = cur_cfg.get("order")
    seasonal = cur_cfg.get("seasonal_order")

    if order and seasonal:
        temp_cfg = {"name": "existing_config", "train_length": 800, "val_length": 60}
        result = try_config(device_id, df_device, temp_cfg)
        if result["success"]:
            return (device_id, result, True, config)

    device_results = []
    for tier in CONFIG_TIERS:
        result = try_config(device_id, df_device, tier)
...
def main():
    ...
    parser.add_argument("--devices", type=str, default=None,
                        help="Comma-separated device IDs to optimize (default: all failing)")
    args = parser.parse_args()

    logger.info("=" * 80)
    logger.info("SARIMAX OPTIMIZATION")
    logger.info(f"Targets: MAE<={TARGET_MAE_MAX}, RMSE<={TARGET_RMSE_MAX}, MAPE<={TARGET_MAPE_MAX}%, R2>={TARGET_R2_MIN}")
    logger.info("=" * 80)
    ...
    tasks = [(dev, df, config) for dev in target_devices]
    ...
    for device_id, result, success, _ in results_list:
        ...
        save_device_config(device_id, config, result)
        if success:
            optimized_count += 1
        else:
    ...
    opt_path = OUT_DIR / "sarimax_optimization_results.csv"
